...
def get_installed_minecraft_versions(directory: str):
...
def get_installed_mods(directory: str) -> dict:
    ...
    for file in os.listdir(directory):
        if file.endswith('.jar'):
            ...
            #Remove leading zeroes from each portion of the version i.e. 1.01.0 -> 1.1.0
...
def download_mod(url: str, filename: str):
...
versions = get_installed_minecraft_versions(versions_dir)
...
for mod in mods_to_update:
    download_mod(up_to_date_versions.get(mod).get('downloadUrl'), f"{mods_dir}{mod}.jar")